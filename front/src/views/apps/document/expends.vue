		...
		<div class="page-header">
			<div class="flex justify-space-between">
				<el-breadcrumb separator="/" class="mb-10">
					...
					<el-breadcrumb-item>Зарлагын баримтууд</el-breadcrumb-item>
				</el-breadcrumb>
			</div>
		</div>
			...
			<table class="styled striped hover">
				...
				<tbody>
					<tr v-for="item in invoices" :key="item.id" :style="{'cursor': 'pointer'}">
						<td @click="goOrder(item.expend_number)">
						...
						<td @click="goOrder(item.expend_number)">
							<div class="item-box item-customer">
								<h4 class="m-0 o-080">{{item.date.replaceAll('T', ' ').replaceAll('.000Z', '')}}</h4>
							</div>
						</td>
                        <td @click="goOrder(item.expend_number)">
							<div class="item-box item-customer">
								<h4 class="m-0 o-080">{{item.expend_number}}</h4>
							</div>
						</td>
                        <td @click="goOrder(item.expend_number)">
							<div class="item-box item-customer">
								<h4 class="m-0 o-080">{{item.company}}</h4>
                                <p class="m-0 o-060">{{item.email}}</p>
							</div>
						</td>
						<td>
...
export default {
	...
	methods: {
		goOrder(ordernumber) {
			this.$router.push({name:'expend', params: {id: ordernumber}})
		},
		...
		getOrders(datetime) {
			...
			if(token) {
				this.$axios({
					method: 'post',
					url: rts.$appUrl +`/api/expend/get-expends`,
					headers: {
		...
		searchOrder() {
			if(this.search !== '') {
				...
				if(token) {
					this.$axios({
						method: 'post',
						url: rts.$appUrl +`/api/expend/search-expend`,
						headers: {
							"Authorization": `Bearer ${token}`
						}, 
						data: {
